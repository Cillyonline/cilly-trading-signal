...
| Dependency/container scan workflow | Partial; visibility workflow exists and runs on PRs/pushes/schedule/manual dispatch, and threshold review policy is documented; reviewed current output and acceptance are still required before production-like exposure | `.github/workflows/security-scans.yml`, `docs/ENGINEERING_WORKFLOW.md#security-scan-workflow`, `docs/SECURITY_SCAN_REVIEW_POLICY.md` |
