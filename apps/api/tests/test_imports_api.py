...
def test_analyze_import_returns_no_setup_for_insufficient_history(client: TestClient) -> None:
    ...
    assert "insufficient_candle_history" in result["signal"]["no_trade_reasons"]
    ...
    assert signals_response.status_code == 200
    assert "insufficient_candle_history" in signals_response.json()[0]["no_trade_reasons"]
    assert signals_response.json()[0]["next_action"]
    assert detail_response.status_code == 200
    assert "insufficient_candle_history" in detail_response.json()["no_trade_reasons"]
